...
public class TestEstudiante {
    ...
    public static void main(String[] args) {
        ...
        ordenardeMayoraMenor(estudiantes,n);
    ...
    public static void mostrarListado(Estudiante est[], int spa){
        for(int i=0;i<spa;i++){
            System.out.println(est[i].toString());
        }
    }
    
    public static void ordenardeMayoraMenor(Estudiante est[], int spa){
        for(int i=0;i<spa;i++){
            for(int j=0;j<spa-1;j++){
                if(est[j].getNota()<=est[j+1].getNota()){
                    //variable donde se guardara la auxiliar
                    Estudiante auxiliar;
                   //array auxiliar
                    auxiliar=est[j];
                 //se guardara en el est y contara de uno en uno   
                 est[j]=est[j+1];
                 //var axiliar
                 est[j+1]=auxiliar;
                }
